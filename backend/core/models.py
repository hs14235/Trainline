...
class TrainTrip(models.Model):
    ...
    origin_station = models.CharField(
        max_length=50, blank=True, null=True, db_column="departure_airport"
    )
    destination_station = models.CharField(
        max_length=50, blank=True, null=True, db_column="arrival_airport"
    )
...
class Passenger(models.Model):
    ...
    membership_level = models.ForeignKey(
        MembershipLevel, on_delete=models.PROTECT, null=True, blank=True
    )
...
class Seat(models.Model):
    ...
    travel_class = models.CharField(
        max_length=16,
        choices=[("standard", "Standard"), ("first", "First"), ("sleeper", "Sleeper")],
    )
...
class Notification(models.Model):
    ...
    train_trip = models.ForeignKey(
        TrainTrip, on_delete=models.CASCADE, null=True, blank=True, db_column="flight_id"
    )
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    membership_level = models.ForeignKey(
        "MembershipLevel", null=True, blank=True, on_delete=models.SET_NULL
    )
    membership_points = models.IntegerField(default=0)
...
class ChatMessage(models.Model):
    train_trip = models.ForeignKey(
        TrainTrip, models.DO_NOTHING, blank=True, null=True, db_column="flight_id"
    )
